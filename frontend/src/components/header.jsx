...
const Header = ({ setOpenMenu }) => {
  ...
  return (
    <nav className="sticky top-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-white/10">
      <div className=" px-5 md:px-6 xl:px-6 py-2 md:py-3 xl:py-4">
          ...
          <div className="hidden md:flex items-center space-x-4 md:space-x-6 xl:space-x-8">
            {navItems.map((item, index) => (
              <a
                key={index}
                href={`#${item.header}`}
                className="text-xs md:text-sm xl:text-base text-gray-300 hover:text-white transition-colors duration-200 relative group"
              >
                {item.header}
